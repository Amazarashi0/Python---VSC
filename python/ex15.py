...
'open(input(Type in filename >)).read()'\n
Type the name of the file you want to open and read
>""")).read())

# 打开一个文件过程：open函数接收一个参数（文件名），并且返回一个值，然后将这个值赋给一个变量(txt)
# 读取该文件：在该变量(txt)上调用一个read函数
